...
import { PlayerInput7ScreenProps } from "../types/navigation.types";
import { COLORS } from "../constants/colors";
import ScreenHeader from "../components/common/ScreenHeader";
import { MatchFormat } from "../types";

const PlayerInput7Screen: React.FC<PlayerInput7ScreenProps> = ({
  navigation,
}) => {
  const [playerNames, setPlayerNames] = useState<string[]>(Array(7).fill(""));
  ...
  const handleStartTournament = () => {
    ...
    if (emptyNames) {
      Alert.alert("Error", "Please fill in all 7 player names");
      return;
    }
    navigation.navigate("DoubleElim7", {
      playerNames,
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        <ScreenHeader
          title="Double Elimination (7)"
          subtitle="Enter 7 Player Names"
        />
        ...
        >
          <Text style={styles.startButtonText}>Start DE-7 Tournament</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
...
export default PlayerInput7Screen;
